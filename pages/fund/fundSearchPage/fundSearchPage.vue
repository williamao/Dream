...
<script>
	...
	export default {
		...
		methods: {
			...
			//执行搜索
			doSearch(keyword) {				
				if(keyword.keyCode==null){
					return false;
				}
				const db = wx.cloud.database() //初始化数据库
				db.collection('fundData').add({
				      data: {
				        fundName: keyword.keyword,
						fundCode: keyword.keyCode,
				      },
				      success: res => {
				        uni.showToast({
				        	title: "添加成功！",
