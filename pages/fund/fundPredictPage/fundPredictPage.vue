			...
			init() {
				...
				uni.request({
					...
					success: (res) => {
						...
						this.ec.option.series[1].data = dataList.map((item) =>
							(+item[2]).toFixed(2)
						);
						let aa = this.handle_num(this.ec.option.series[0].data);
						this.DWJZ = res.data.Expansion.DWJZ;
						this.ec.option.yAxis[0].min = -aa;
						this.ec.option.yAxis[0].max = aa;
						this.ec.option.yAxis[0].interval = aa / 4;
						this.ec.option.yAxis[1].min = -aa;
						this.ec.option.yAxis[1].max = aa;
						this.ec.option.yAxis[1].interval = aa / 4;
						// 
					}
				});
			},
