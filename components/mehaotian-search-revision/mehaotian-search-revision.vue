	...
	<view class="serach">
		<view class="content" :style="{'border-radius':radius+'px'}">
			...
			<view class="content-box" :class="{'center':mode === 2}" >
				...
				<input focus :placeholder="placeholder" @input="inputChange" confirm-type="search" @confirm="triggerConfirm" class="input" :class="{'center':!active && mode === 2}" :focus="isFocus" v-model="inputVal" @focus="focus" @blur="blur"/>
...
<!-- 		<view  v-if="button === 'outside'" class="button" :class="{'active':show||active}" @click="search">
			<view class="button-item">{{!show?searchName:'添加'}}</view>
		</view> -->
	</view>
</template>
...
.serach {
	...
	.content {
		...
		.serachBtn {
			...
			padding: 0 30upx;
			//HM修改 按钮背景色
			background:linear-gradient(to right,#ff000c,#ff000c);
